...
// Canonical spend/savings rendering shared by the two Wendl harness builds:
// the OpenClaw plugin (plugins/openclaw) and the nanoclaw built-in (plugins/nanoclaw).
// Pure functions only — NO fetch, env, logger, or harness types — so the /limits +
// /stats math has ONE source of truth and the two harnesses can't drift. Each harness
// keeps its own fetch/config/labels and wraps these with its own title + footer.
//
// This file is the source of truth. `make sync-shared` vendors a copy into each
// consumer (which must ship self-contained: OpenClaw -> public repo, nanoclaw -> fork).

export interface SpendKey { key_alias?: string; spend?: number; max_budget?: number; }
export interface SpendLog { spend?: number; prompt_tokens?: number; completion_tokens?: number; }
export interface Baseline { name: string; in: number; out: number; }
export interface Totals { inTok: number; outTok: number; actual: number; n: number; }
...
export function renderBudgetLines(
  keys: SpendKey[],
  opts: { order?: string[]; labels?: Record<string, string> } = {},
): string[] {
  ...
  if (shown.length === 0) return ['_No budgeted keys found._'];
  return shown.map((k) => {
...
export function sumLogs(logs: SpendLog[]): Totals {
  const t: Totals = { inTok: 0, outTok: 0, actual: 0, n: 0 };
  for (const l of logs) {
...
export function renderSavingsLines(
  t: Totals,
  opts: { baselines?: Baseline[]; spentLabel?: string } = {},
): string[] {
